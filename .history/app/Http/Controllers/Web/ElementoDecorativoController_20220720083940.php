...
class ElementoDecorativoController extends Controller {
    ...
    public function elemento_decorativo($id) {
        try {
        ...
        return view('web.pages.elemento_decorativo.web_elemento_decorativo', compact('Products'));
    ...
    public function elemento_decorativo_($id,$id2) {
        try {
    ...
    public function hide_tirador_encimera(Request $request) {
        return view('web.pages.elemento_decorativo.ajax.table_encimera');
    }
    ...
    public function change_tirador_encimera(Request $request)
    {
        ...
        $html.='<div>
                <label class="radio-img" data-toggle="tooltip" data-placement="top"
                    title="xxx">
                    <input type="radio" name="color_puertas" onclick="modalAbrirImagen('."'".$Imagen[0]->url_image."'".', '."'ENZIMERA'".')">
                    ...
                    $html.='<label for="group_1">Código: '.$Imagen[0]->nombre.'</label>
                        <div>
                            <label class="radio-img" data-toggle="tooltip" data-placement="top"
                                ...
                                <input type="radio" name="color_puertas" onclick="modalAbrirImagen('."'".$Imagen[0]->url_image."'".', '."'TIRADOR'".')">
                                <img  style="width: 60%;" src="'.$Imagen[0]->url_image.'">
                            </label>
                        </div>';
        ...
        $Imagen = DB::table('mc_tirador_unero')
                    ...
                    $html.='<label for="group_1">Código: '.$Imagen[0]->nombre.'</label>
                        <div>
                            <label class="radio-img" data-toggle="tooltip" data-placement="top"
                                title="'.$Imagen[0]->nombre.'">
                                <input type="radio" name="color_puertas" onclick="modalAbrirImagen('."'".$Imagen[0]->url_image."'".', '."'UNERO'".')">
                                <img  style="width: 100%;" src="'.$Imagen[0]->url_image.'">
                            </label>
                        </div>';
    ...
    public function html_imagen_modelo_puerta_decorativo(Request $request)
    {
        $Imagen = DB::table('mc_colores')
                         ->where('id', $request->txt_id)
                         ->get();
        $html="";
        $html.='<label for="group_1">'.$Imagen[0]->name_color.'</label>
            <div>
                ...
                <label class="radio-img" data-toggle="tooltip" data-placement="top" title="">
                    <input type="radio" name="color_puertas"   onclick="modalAbrirImagen('."'".$Imagen[0]->url_image."'".', '."'PUERTA'".')">
                    <img style="width: 100%;" src="'.$Imagen[0]->url_image.'">
                </label>
            </div>';
    ...
    public function html_imagen_color_puerta_(Request $request)
    {
        $Imagen = DB::table('mc_colores')
                    ...
                    $html.='<label for="group_1">'.$Imagen[0]->name_color.'</label>
                        <div>
                            ...
                            <label class="radio-img" data-toggle="tooltip" data-placement="top"
                                title="'.$Imagen[0]->name_color.'">
                                <input type="radio" name="color_puertas" onclick="modalAbrirImagen('."'".$Imagen[0]->url_image."'".', '."'COLOR'".')">
                                <img style="width: 100%;" src="'.$Imagen[0]->url_image.'">
                            </label>
                        </div>';
    ...
    public function html_imagen_colores(Request $request) {
        $rdData = DB::select("
                        ...
                        ORDER BY xs.id ASC", [$request->id_producto, $request->id_modelo_puerta]);
    ...
    return view('web.pages.elemento_decorativo.ajax.table_colores')
    ->with(compact('rdData'));
    //  return $html;
    }
 
    public function calcular_total_precio_medida_decorativo(Request $request) {
        $price = DB::select("
                            SELECT mp.* FROM medida mp   
                            WHERE   mp.id_imagen = ?
                                    AND mp.ancho=?
                                    AND mp.alto=?
                            ORDER BY mp.id_medida ASC", [$request->id_imagen, $request->ancho, $request->alto]);
