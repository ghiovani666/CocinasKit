...
class MuebleCompletoController extends Controller {
 ...
 {        
  ...
  public function mueble_puertas_listar_(Request $request) {
    ...
    $cbColores = DB::select("
                    SELECT M.id as id_xx, M.id_modelo_puerta as id_mode,T.id_color FROM mc_colores M
    ...
    $rowData_ = DB::select("SELECT mp.id as id_modelo_puerta , mp.nombre, mp.url_image FROM mc_modelo_puerta mp INNER JOIN mc_modelo_puerta_intermedio mps ON mp.id = mps.id_modelo_puerta
