...
class ComposicionOfertaController extends Controller {
    ...
    public function composicion_oferta_listar_combobox(Request $request) {
      ...
      if($request->id_producto){// (numero)// para actualizar
        ...
        $cbCopete = DB::select("SELECT * FROM mc_enzimera M LEFT JOIN (SELECT cin.id_enzimera,cin.id_producto FROM mc_enzimera c INNER JOIN mc_enzimera_intermedio cin ON c.id=cin.id_enzimera WHERE cin.id_producto = ?) AS T ON M.id=T.id_enzimera ORDER BY M.id asc", [$request->id_producto]);
