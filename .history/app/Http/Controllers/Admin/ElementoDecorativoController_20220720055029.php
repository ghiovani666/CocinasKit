...
class ElementoDecorativoController extends Controller {
    ...
    public function elemento_decorativo_listar_combobox(Request $request) {
      ...
      $cbColorRegleta = DB::select("SELECT * FROM mc_colores M LEFT JOIN (SELECT cin.id_color,cin.id_producto FROM mc_colores c INNER JOIN mc_colores_intermedio cin ON c.id=cin.id_color WHERE cin.id_producto = ?)  AS T ON M.id=T.id_color WHERE M.id_cat=2 ORDER BY M.id asc", [$request->id_producto]);
