...
class ComposicionOfertaController extends Controller {
    ...
    public function composicion_oferta_listar_combobox(Request $request) {

      $validar = DB::select("SELECT * FROM mc_colores M LEFT JOIN (SELECT cin.id_color,cin.id_producto FROM mc_colores c INNER JOIN mc_colores_intermedio cin ON c.id=cin.id_color WHERE cin.id_producto = ?)  AS T ON M.id=T.id_color WHERE M.id_cat=1 ORDER BY M.id asc", [$request->id_producto]);
      //false- true: es nuemro cuando es true
      //false: cuando es false
      if($request->id_producto){// (numero)// para actualizar
        $cbColores = DB::select("SELECT * FROM mc_colores M LEFT JOIN (SELECT cin.id_color,cin.id_producto FROM mc_colores c INNER JOIN mc_colores_intermedio cin ON c.id=cin.id_color WHERE cin.id_producto = ?)  AS T ON M.id=T.id_color WHERE M.id_cat=1 ORDER BY M.id asc", [$request->id_producto]);
        ...
        $cbEnzimera = DB::select("SELECT * FROM mc_enzimera M LEFT JOIN (SELECT cin.id_enzimera,cin.id_producto FROM mc_enzimera c INNER JOIN mc_enzimera_intermedio cin ON c.id=cin.id_enzimera WHERE cin.id_producto = ?) AS T ON M.id=T.id_enzimera ORDER BY M.id asc", [$request->id_producto]);
        $cbCopete = DB::select("SELECT * FROM mc_copete M LEFT JOIN (SELECT cin.id_copete,cin.id_producto FROM mc_copete c INNER JOIN mc_copete_intermedio cin ON c.id=cin.id_copete WHERE cin.id_producto = ?) AS T ON M.id=T.id_copete ORDER BY M.id asc", [$request->id_producto]);
      ...
      }else{//(false)//para crear
        $cbColores = DB::select("SELECT * FROM mc_colores M WHERE M.id_cat=1 ORDER BY M.id asc");
        $cbTirador = DB::select("SELECT * FROM mc_tirador M ORDER BY M.id asc");
        $cbEnzimera = DB::select("SELECT * FROM mc_enzimera M ORDER BY M.id asc");
        $cbCopete = DB::select("SELECT * FROM mc_copete M  ORDER BY M.id asc");
        $cbModelo = DB::select("SELECT * FROM mc_modelo_puerta M ORDER BY M.id asc");
      }
      return view('admin.pages.composicion_oferta.ajax.opcionesComboBox')
      ->with(compact('cbColores'))
      ->with(compact('cbTirador'))
      ->with(compact('cbEnzimera'))
      ->with(compact('cbCopete'))
 ...
 {        
          ...
          $arraysCopete = $request->nameCopete;
              ...
              $id_producto = DB::table('products')->insertGetId([
               ...
               DB::table('menu_items')->where('id_item', '43')->update(['numbers' => $count]);
                ...
                if($file5 != NULL){
                  ...
                  $result = DB::table('imagen')->insert([
                    ...
                    if($arraysCheck_1 != NULL){
                        $data1 = DB::select("SELECT * FROM mc_colores M WHERE M.id_cat=1");
                ...
                if($arraysCheck_3 != NULL){
                  $data3 = DB::select("SELECT * FROM mc_enzimera M");
                  foreach($data3 as $value) {
                      DB::table('mc_enzimera_intermedio')->insert(['id_producto' => $id_producto,'id_enzimera' => $value->id]);
                    }
                }else{
                  if (!empty($arraysEnzimera) && count($arraysEnzimera) > 0)
                ...
                //  :::::::::::::::::::: 5: COPETE :::::::::::::::::::::::::::::::
                if($arraysCheck_5 != NULL){
                  $data5 = DB::select("SELECT * FROM mc_copete M");
                  foreach($data5 as $value) {
                      DB::table('mc_copete_intermedio')->insert(['id_producto' => $id_producto,'id_copete' => $value->id]);
                    }
                }else{
                  if (!empty($arraysCopete) && count($arraysCopete) > 0)
                  foreach ($arraysCopete as $value) {
                      DB::table('mc_copete_intermedio')->insert(['id_producto' => $id_producto,'id_copete' => $value]);
                    }
                }
          ...
          case 'ACTUALIZAR': 
                ...
                if($file0 != NULL &&  $id_imagen_0 != NULL){
                  ...
                  if($file1 != NULL &&  $id_imagen_1 != NULL){
                    ...
                    if(file_exists(str_replace('/img/', 'img/',  $url_imagen[0]->url_image))){
                      ...
                      if($file1 != NULL){
                        $filename1  =  time() .'_'.$file1->getClientOriginalName();
                        $path1 = "img/mc_composicion_oferta/".$request->id_producto;
                        $file1->move($path1,$filename1);
        
                        DB::table('imagen')->insert([
                            'id_product'  =>  $request->id_producto, 
                            'url_image'   => '/img/mc_composicion_oferta/'.$request->id_producto.'/'.$filename1, 
                            'created_at'  =>  date("Y-m-d H:i:s"),
                            'updated_at'  =>  date("Y-m-d H:i:s")
                          ]);
                      }
                    ...
                    if($file2 != NULL &&  $id_imagen_2 != NULL){
                      ...
                      if(file_exists(str_replace('/img/', 'img/',  $url_imagen[0]->url_image))){
                        ...
                        $result  =  DB::table('imagen')
                          ...
                          if($file2 != NULL){
                            $filename2  =  time() .'_'.$file2->getClientOriginalName();
                            $path2 = "img/mc_composicion_oferta/".$request->id_producto;
                            $file2->move($path2,$filename2);
            
                            DB::table('imagen')->insert([
                                'id_product'  =>  $request->id_producto, 
                                'url_image'   => '/img/mc_composicion_oferta/'.$request->id_producto.'/'.$filename2, 
                                'created_at'  =>  date("Y-m-d H:i:s"),
                                'updated_at'  =>  date("Y-m-d H:i:s")
                              ]);
                          }
                      ...
                      if($file3 != NULL &&  $id_imagen_3 != NULL){
                        ...
                        if(file_exists(str_replace('/img/', 'img/',  $url_imagen[0]->url_image))){
                          ...
                          $result  =  DB::table('imagen')
                            ...
                            if($file3 != NULL){
                              $filename3  =  time() .'_'.$file3->getClientOriginalName();
                              $path3 = "img/mc_composicion_oferta/".$request->id_producto;
                              $file3->move($path3,$filename3);
              
                              DB::table('imagen')->insert([
                                  'id_product'  =>  $request->id_producto, 
                                  'url_image'   => '/img/mc_composicion_oferta/'.$request->id_producto.'/'.$filename3, 
                                  'created_at'  =>  date("Y-m-d H:i:s"),
                                  'updated_at'  =>  date("Y-m-d H:i:s")
                                ]);
                            }
                        ...
                        if($file4 != NULL &&  $id_imagen_4 != NULL){
                          ...
                          if(file_exists(str_replace('/img/', 'img/',  $url_imagen[0]->url_image))){
                            ...
                            $result  =  DB::table('imagen')
                              ...
                              if($file4 != NULL){
                                $filename4  =  time() .'_'.$file4->getClientOriginalName();
                                $path4 = "img/mc_composicion_oferta/".$request->id_producto;
                                $file4->move($path4,$filename4);
                
                                DB::table('imagen')->insert([
                                    'id_product'  =>  $request->id_producto, 
                                    'url_image'   => '/img/mc_composicion_oferta/'.$request->id_producto.'/'.$filename4, 
                                    'created_at'  =>  date("Y-m-d H:i:s"),
                                    'updated_at'  =>  date("Y-m-d H:i:s")
                                  ]);
                              }
                          ...
                          if($file5 != NULL &&  $id_imagen_5 != NULL){
                            ...
                            if(file_exists(str_replace('/img/', 'img/',  $url_imagen[0]->url_image))){
                              ...
                              $result  =  DB::table('imagen')
                                ...
                                if($file5 != NULL){
                                  $filename5  =  time() .'_'.$file5->getClientOriginalName();
                                  $path5 = "img/mc_composicion_oferta/".$request->id_producto;
                                  $file5->move($path5,$filename5);
                  
                                  DB::table('imagen')->insert([
                                      'id_product'  =>  $request->id_producto, 
                                      'url_image'   => '/img/mc_composicion_oferta/'.$request->id_producto.'/'.$filename5, 
                                      'created_at'  =>  date("Y-m-d H:i:s"),
                                      'updated_at'  =>  date("Y-m-d H:i:s")
                                    ]);
                                }
                              ...
                              //  :::::::::::::::::::: 3: ENZIMERA :::::::::::::::::::::::::::::::
                               ...
                               if($arraysCheck_3 != NULL){
                                $data3 = DB::select("SELECT * FROM mc_enzimera M");
                                foreach($data3 as $value) {
                                    DB::table('mc_enzimera_intermedio')->insert(['id_producto' => $request->id_producto,'id_enzimera' => $value->id]);
                                  }
                              }else{
                                if (!empty($arraysEnzimera) && count($arraysEnzimera) > 0)
                                foreach ($arraysEnzimera as $value) {
                              ...
                              //  :::::::::::::::::::: 5: COPETE :::::::::::::::::::::::::::::::
                              DB::table('mc_copete_intermedio')->where('id_producto', '=', $request->id_producto)->delete();
                              if($arraysCheck_5 != NULL){
                               $data5 = DB::select("SELECT * FROM mc_copete M");
                               foreach($data5 as $value) {
                                   DB::table('mc_copete_intermedio')->insert(['id_producto' => $request->id_producto,'id_copete' => $value->id]);
                                 }
                             }else{
                               if (!empty($arraysCopete) && count($arraysCopete) > 0)
                               foreach ($arraysCopete as $value) {
                                   DB::table('mc_copete_intermedio')->insert(['id_producto' => $request->id_producto,'id_copete' => $value]);
                                 }
                             }
            ...
            case 'ELIMINAR': 
              ...
              try {
                  ...
                  if(file_exists('img/mc_composicion_oferta/'.$request->id_producto)){
                    $folderPath=public_path('img/mc_composicion_oferta/'.$request->id_producto);
