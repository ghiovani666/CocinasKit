...
class MuebleCompletoController extends Controller {
 ...
 {        
  ...
  public function mueble_puertas_listar_(Request $request) {
    ...
    $cbColores = DB::select("
                  SELECT * FROM mc_colores M
                  LEFT JOIN (SELECTcin.id_color,cin.id_producto FROM mc_colores c INNER JOIN md_modelo_puerta_color_intermedio cin ON c.id = cin.id_color
                    WHERE cin.id_producto = ? ) AS T ON M.id = T.id_color
                  ORDER BY M.id ASC ", [$request->id_producto]);
