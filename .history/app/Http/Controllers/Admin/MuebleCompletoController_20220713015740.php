...
class MuebleCompletoController extends Controller {
 ...
 {        
  ...
  public function mueble_puertas_listar_(Request $request) {
    ...
    $cbColores = DB::select("SELECT * FROM mc_colores M LEFT JOIN (SELECT cin.id_color,cin.id_producto FROM mc_colores c INNER JOIN md_modelo_puerta_color_intermedio cin ON c.id=cin.id_color WHERE cin.id_producto = ?)  AS T ON M.id=T.id_color ORDER BY M.id asc", [$request->id_producto]);

    $rowData_ = DB::select("SELECT*FROM mc_modelo_puerta mp INNER JOIN mc_modelo_puerta_intermedio mps ON mp.id = mps.id_modelo_puerta
