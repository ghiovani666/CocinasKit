...
class ComposicionOfertaController extends Controller {
    ...
    public function composicion_oferta() {
      ...
      $cbColores = DB::table('mc_colores')->get();

      return view('admin.pages.composicion_oferta.admin_composicion_oferta')->with(compact('cb_listCategoria'))->with(compact('cbColores'));
    }
 ...
 {        
                          ...
                          if(file_exists(str_replace('/img/', 'img/',  $url_imagen[0]->url_image))){
                            ...
                            if(file_exists(str_replace('/img/', 'img/',  $url_imagen[0]->url_image))){
                              ...
                              $result  =  DB::table('imagen')
                               ...
                               DB::table('products')->where("id",$request->id_producto)->update(['pro_info' => $request->txt_descripcion,'updated_at' =>date("Y-m-d H:i:s")]); 
                               $result  = DB::table('medida')->where("id_imagen",$id_imagen_0)->update(['price' => $request->txt_price,'updated_at' =>date("Y-m-d H:i:s")]); 
    ...
    public function composicion_oferta_editar(Request $request) {
      ...
      $rowColor = DB::select("
          ...
          id_producto, 
          id_color
          FROM mc_colores_intermedio
          WHERE id_producto=?
          ORDER BY id_color asc ", [$request->id_producto]);

      return json_encode([$rowData_,$rowColor]) ;
    }
