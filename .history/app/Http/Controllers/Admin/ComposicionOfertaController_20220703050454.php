...
class ComposicionOfertaController extends Controller {
    ...
    public function composicion_oferta_listar(Request $request) {
      ...
      return View::make('admin.pages.composicion_oferta.ajax.tablaProducto', $rowData_)
->nest('admin.pages.composicion_oferta.admin_composicion_oferta', $rowData_)

     // return view('admin.pages.composicion_oferta.ajax.tablaProducto')->nest('admin.pages.composicion_oferta.admin_composicion_oferta')->with(compact('rowData_'));
    }

    
    public function composicion_oferta_listar_combobox(Request $request) {

      $cbColores = DB::select("SELECT * FROM mc_colores c LEFT JOIN mc_colores_intermedio cin ON c.id=cin.id_color WHERE cin.id_producto=? ORDER BY c.id asc", [$request->id_producto]);
      $cbTirador = DB::select("SELECT * FROM mc_tirador mt LEFT JOIN mc_tirador_intermedio mti ON mt.id=mti.id_tirador WHERE mti.id_producto=? ORDER BY mt.id asc", [$request->id_producto]);
      $cbEnzimera = DB::select("SELECT * FROM mc_enzimera mt LEFT JOIN mc_enzimera_intermedio mti ON mt.id=mti.id_enzimera WHERE mti.id_producto=? ORDER BY mt.id asc", [$request->id_producto]);
