 ...
 {        
    ...
    try {
      ...
      if($isValues_ > 0){
        ...
        $cantidad = DB::select("SELECT count(*) AS cantidad FROM medida WHERE ancho=? AND alto=? AND fondo=? AND id_imagen=? ", [$request->txt_ancho_, $request->txt_alto_, $request->txt_id_imagen_]);
