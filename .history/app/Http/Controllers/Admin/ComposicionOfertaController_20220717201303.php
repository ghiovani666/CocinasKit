...
class ComposicionOfertaController extends Controller {
    ...
    public function composicion_oferta_listar_combobox(Request $request) {
      ...
      $validar = DB::select("SELECT * FROM mc_colores M LEFT JOIN (SELECT cin.id_color,cin.id_producto FROM mc_colores c INNER JOIN mc_colores_intermedio cin ON c.id=cin.id_color WHERE cin.id_producto = ?)  AS T ON M.id=T.id_color ORDER BY M.id asc", [$request->id_producto]);
      ...
      if($request->id_producto){// (numero)// para actualizar
        ...
        $cbColores = DB::select("SELECT * FROM mc_colores M ORDER BY M.id asc");
