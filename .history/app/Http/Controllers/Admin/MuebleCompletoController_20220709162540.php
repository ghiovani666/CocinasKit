...
class MuebleCompletoController extends Controller {
 ...
 {        
    ...
    // public function filtro_combobox_cbOpcionMueble(Request $request) {
    ...
    // WHERE p.id_item IN (SELECT id_item FROM menu_items WHERE id_cat in (SELECT id_cat FROM menu_category WHERE id_menu=4)) 
    // AND p.id_item = ?
    // ORDER BY p.id asc ", [$request->txt_id_item]);

    //   return json_encode(['data' => $rowData_]) ;
    // }

    public function listar_medidas(Request $request) 
    {        
      // $txt_radio_cp = $request->txt_radio_cp;//RADIO BUTTOM
          
      // $txt_mueble_1 = $request->txt_mueble_1;//SELECT
      // $txt_mueble_2 = $request->txt_mueble_2;//INPUT
      ...
      // var_dump($txt_mueble_1);
      // var_dump($txt_mueble_2);
