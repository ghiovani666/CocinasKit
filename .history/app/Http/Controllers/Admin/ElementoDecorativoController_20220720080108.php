...
class ElementoDecorativoController extends Controller {

    //:::::::::::::::::::::::::: CRUD COMPOSICION OFERTA::::::::::::::::::::::::::

    public function elemento_decorativo() {
        // :::::::::::::::::: COMBO DEPENDIENTES SOLO PARA INICIAR LA CATEGORIA ::::::::::::::::::::
      $cbCategoria = DB::select("SELECT * FROM menu_category M  WHERE M.id_menu=4 AND M.id_cat=19 ORDER BY M.id_cat asc");
      return view('admin.pages.elemento_decorativo.admin_elemento_decorativo')
    ...
    public function elemento_decorativo_listar(Request $request) {
      ...
      WHERE p.id_item IN (54,55) AND p.id_item = ?
      ...
      INNER JOIN menu_items mi on mc.id_cat=mi.id_cat
      INNER JOIN products p on mi.id_item=p.id_item
      WHERE p.id_item IN (54,55) AND mc.id_cat = ?
      ORDER BY p.id desc ", [$request->id_categoria]);
      ...
      INNER JOIN menu_items mi on mc.id_cat=mi.id_cat
      INNER JOIN products p on mi.id_item=p.id_item
      WHERE p.id_item IN (54,55)
      ORDER BY p.id desc  ");

      }

      return view('admin.pages.elemento_decorativo.ajax.tablaProducto')->with(compact('rowData_'));
    }


    
    public function elemento_decorativo_listar_combobox(Request $request) {
      ...
      $cbColorCostado = DB::select("SELECT * FROM mc_colores M LEFT JOIN (SELECT cin.id_color,cin.id_producto FROM mc_colores c INNER JOIN mc_colores_intermedio cin ON c.id=cin.id_color WHERE cin.id_producto = ?)  AS T ON M.id=T.id_color WHERE M.id_cat=2 ORDER BY M.id asc", [$request->id_producto]);
      $cbColorRegleta = DB::select("SELECT * FROM mc_colores M LEFT JOIN (SELECT cin.id_color,cin.id_producto FROM mc_colores c INNER JOIN mc_colores_intermedio cin ON c.id=cin.id_color WHERE cin.id_producto = ?)  AS T ON M.id=T.id_color WHERE M.id_cat=3 ORDER BY M.id asc", [$request->id_producto]);

      return view('admin.pages.elemento_decorativo.ajax.opcionesComboBox')
      ->with(compact('cbColores'))
      ->with(compact('cbTirador'))
      ->with(compact('cbModelo'))
      ->with(compact('cbTiradorUnero'))

      ->with(compact('cbColorCostado'))
      ->with(compact('cbColorRegleta'));
    }

 public function elemento_decorativo_crear(Request $request) 
 {        
        ...
        $arraysColorCostado = $request->nameColorCostado;
        $arraysColorRegleta = $request->nameColorRegleta;
              ...
              $id_producto = DB::table('products')->insertGetId([
               ...
               DB::table('menu_items')->where('id_item', $txt_id_item)->update(['numbers' => $count]);
                ...
                if($file0 != NULL){
                  ...
                  $path = "img/mc_elemento_decorativo/".$id_producto;
                  $file0->move($path,$filename);
  
                  $id_imagen__ = DB::table('imagen')->insertGetId([
                      'id_product' =>$id_producto, 
                      'url_image' => '/img/mc_elemento_decorativo/'.$id_producto.'/'.$filename, 
                      'created_at' => date("Y-m-d H:i:s"),
                      'updated_at' =>date("Y-m-d H:i:s")
                    ]);
                }
                      ...
                      if($arraysCheck_5 != NULL){
                        $data5 = DB::select("SELECT * FROM mc_colores M WHERE M.id_cat=2");
                        foreach($data5 as $value) {
                            DB::table('mc_colores_intermedio')->insert(['id_producto' => $id_producto,'id_color' => $value->id]);
                          }
                    }else{
                      if (!empty($arraysColorCostado) && count($arraysColorCostado) > 0)
                          foreach($arraysColorCostado as $value) {
                            DB::table('mc_colores_intermedio')->insert(['id_producto' => $id_producto,'id_color' => $value]);
                          }
                    }

                      //  :::::::::::::::::::: 6: COLOR REGLETA :::::::::::::::::::::::::::::::
                      if($arraysCheck_6 != NULL){
                        $data6 = DB::select("SELECT * FROM mc_colores M WHERE M.id_cat=3");
                        foreach($data6 as $value) {
                            DB::table('mc_colores_intermedio')->insert(['id_producto' => $id_producto,'id_color' => $value->id]);
                          }
                    }else{
                      if (!empty($arraysColorRegleta) && count($arraysColorRegleta) > 0)
                          foreach($arraysColorRegleta as $value) {
                            DB::table('mc_colores_intermedio')->insert(['id_producto' => $id_producto,'id_color' => $value]);
                          }
                    }
          ...
          case 'ACTUALIZAR': 
            ...
            try {
                ...
                if($file0 != NULL &&  $id_imagen_0 != NULL){
                    ...
                    $filename  =  time() .'_'.$file0->getClientOriginalName(); 
                    $path = "img/mc_elemento_decorativo/".$request->id_producto;
                    $file0->move($path,$filename); 
      
                    DB::table('imagen')
                      ->where("id_imagen",$id_imagen_0)
                      ->update([
                        'url_image' => '/img/mc_elemento_decorativo/'.$request->id_producto.'/'.$filename, 
                        'updated_at' =>date("Y-m-d H:i:s")
                      ]); 
                  }
                    ...
                    //var_dump($arraysColorCostado); exit;
            ...
            case 'ELIMINAR': 
              ...
              try {
                  ...
                  if(file_exists('img/mc_elemento_decorativo/'.$request->id_producto)){
                    $folderPath=public_path('img/mc_elemento_decorativo/'.$request->id_producto);
    ...
    public function elemento_decorativo_editar(Request $request) {
    ...
    public function elemento_decorativo_listar_medidas(Request $request) 
    {        
     $rowData_ = DB::select("
          SELECT
      ...
      return view('admin.pages.elemento_decorativo.ajax.tablaMedida')->with(compact('rowData_'));

    }  

    public function elemento_decorativo_crear_medida(Request $request) 
    {        
    ...
    try {
      $cantidad = DB::select("SELECT count(*) AS cantidad FROM medida WHERE ancho=? AND alto=? AND id_imagen=? ", [$request->txt_ancho_, $request->txt_alto_, $request->txt_id_imagen_]);
      if($isValues_ > 0){
        ...
        if((int)$cantidad[0]->cantidad == 1 || (int)$cantidad[0]->cantidad == 0){
  ...
  public function elemento_decorativo_editar_medida(Request $request) {
  ...
  public function elemento_decorativo_eliminar_medida(Request $request) {
    DB::beginTransaction();
  ...
  public function mueble_puertas_listar_(Request $request) {
    ...
    return view('admin.pages.elemento_decorativo.ajax.tablaProductoAsignar')
    ->with(compact('cbColores'))
  ...
  public function elemento_decorativo_asignar_color(Request $request) 
  {        
...
public function listarDataTableInfo(Request $request) {
  ...
  return view('admin.pages.elemento_decorativo.ajax.tablaProductoFicha')->with(compact('rowData_'));
}
