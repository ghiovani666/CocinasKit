...
function listarDataTable(id_categoria,isValues) {
    $.ajax({
        ...
        data: "id_categoria=" + id_categoria,"isValues=" + isValues
...
$('#uploadForm').on('submit', function(e) {
    ...
    axios.post('mueble_completo_crear',
        ...
        if (response.data.state == "error") {
            ...
            listarDataTable($('select[id=menu_item]').val(),"LISTA_ITEM")
...
$('#menu_categoria').on('change', function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            listarDataTable(0,"LISTA_TODO")
            $('#menu_item').empty();
            $('#menu_item').append('<option value="0" disable="true" selected="true"> <========== Todos ==========> </option>');

            $.each(response.data, function(fetch, regenciesObj) {
                $('#menu_item').append('<option value="' + regenciesObj.id_item + '">' +regenciesObj.names + '</option>');
            })
        }
    });
});
...
$('select[id=menu_item]').on('change', function() {
    const id_item = $('select[id=menu_item]').val();
    listarDataTable(id_item,"LISTA_ITEM")
});
