...
<div class="col-12" style="display: flex;margin-top: 12px;">
    ...
    <label for="lbl_name" class="check_ShowHide" style="margin-left: 20px;margin-top: 2px;">Todos:</label>
    <input type="checkbox" class="check_ShowHide" id="check_5" name="check_5" style="width: 17px;height: 33px;margin-left: 10px;" value="3">
</div>

<div class="btn-group btn-group-toggle" data-toggle="buttons">
    <div class="select2-purple">
        <select name="nameCopete[]" multiple
            class="form-control nameCopete <!-- @error('authors') is-invalid @enderror -->">
            @foreach ($cbCopete as $items)
            <option value="{{ $items->id }}" {{ $items->id == $items->id_copete ? 'selected' : '' }}>
                {{ $items->nombre }}</option>
            @endforeach
        </select>
    </div>
...
<script>
//:::::::::::: CRUD SERVICIOS :::::::::::::::::::::::::::::
$('.nameColor,.nameTirador,.nameEnzimera,.nameModelo,.nameCopete').select2()
</script>
